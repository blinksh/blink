...
lib.MessageManager.prototype.loadMessages = function(
  ...
  if (opt_onError) xhr.onerror = () => opt_onError(xhr);
...
lib.MessageManager.prototype.processI18nAttributes = function(dom) {
  ...
  for (var i = 0; i < nodes.length; i++) this.processI18nAttribute(nodes[i]);
};
...
lib.PreferenceManager.prototype.definePreference = function(
  ...
  if (record) {
    ...
    record = this.prefRecords_[name] = new lib.PreferenceManager.Record(
      name,
      value,
    );
  }
 ...
 */
lib.PreferenceManager.prototype.importFromJson = function(
  json,
  opt_onComplete,
) {
  let pendingWrites = 0;
  ...
  const onComplete = () => {
    if (chrome.runtime.lastError) {
      ...
      if (chrome.runtime.lastError.message.indexOf('MAX_WRITE_OPERATIONS')) {
        console.warn(
          `Will retry save of ${key} after exceeding quota`,
...
lib.TestManager.Log = function(opt_console = console) {
  ...
  ['group', 'groupCollapsed'].forEach(group => {
    ...
    this[group] = this.console_[group] = (label = '') => {
      oGroup(label);
      if (this.save) this.data += this.prefix_ + label + '\n';
  ...
  this.groupEnd = this.console_.groupEnd = () => {
    oGroupEnd();
    if (this.prefixStack_) this.prefix_ = '  '.repeat(--this.prefixStack_);
  };
};
...
lib.TestManager.TestRun.prototype.selectPattern = function(pattern) {
  ...
  for (var i = 0; i < lib.TestManager.Suite.subclasses.length; i++) {
    selectCount += this.selectSuite(
      lib.TestManager.Suite.subclasses[i],
      pattern,
    );
  }
...
lib.TestManager.TestRun.prototype.onUncaughtException_ = function(
  ...
  if (
    message.indexOf('Uncaught lib.TestManager.Result.TestComplete') == 0 ||
...
lib.TestManager.TestRun.prototype.onResultComplete = function(result) {
  ...
  if (result.status != result.PASSED) this.log.error(result.status);
  ...
  if (result.status == result.FAILED) {
    ...
    this.log.error(
      'Unknown result status: ' + result.test.fullName + ': ' + result.status,
    );
    this.panic = true;
    return;
  }
...
lib.TestManager.Result.prototype.assertEQ = function(
  ...
  if (expected instanceof Array)
    if (lib.array.compare(actual, expected)) return;
...
lib.TestManager.Result.prototype.assert = function(actual, opt_name) {
  ...
  this.fail(
    'assert' + name + ': ' + this.getCallerLocation_(1) + ': ' + String(actual),
  );
};
...
lib.encodeUTF8 = function(str) {
  ...
  for (var i = 0; i < str.length; i++) {
    ...
    if (c <= 0x7f) {
      ...
      /* if (c <= 0x10FFFF) */ ret += String.fromCharCode(0xf0 | (c >>> 18));
      bytesLeft = 3;
    }
...
lib.wc.charWidthDisregardAmbiguous = function(ucs) {
  ...
  if (ucs < 0x7f) {
    ...
    else if (ucs == 0) return lib.wc.nulWidth; /* if (ucs < 0x20) */
    else return lib.wc.controlWidth;
  }
...
lib.resource.add('hterm/changelog/version', 'text/plain', '1.77' + '');

lib.resource.add('hterm/changelog/date', 'text/plain', '2018-01-05' + '');
...
hterm.Frame.prototype.show = function() {
  ...
  if (false) {
    ...
    header.style.cssText =
      ...
      ';' +
      'color: ' +
      this.terminal_.getBackgroundColor() +
      ';' +
      'font-size: 16px;' +
      'font-family: ' +
      this.terminal_.getFontFamily();
...
hterm.Keyboard.KeyPattern = function(spec) {
  ...
  hterm.Keyboard.KeyPattern.modifiers.forEach(
    function(mod) {
...
hterm.Parser.prototype.parseKeySequence = function() {
  ...
  while (this.pos < this.source.length) {
    ...
    if (token.type == 'integer') {
      ...
      if (
        ucValue in hterm.Parser.identifiers.modifierKeys &&
        ...
        ucValue in hterm.Parser.identifiers.keyCodes &&
...
hterm.Parser.prototype.parseKeyAction = function() {
  ...
  if (token.type == 'identifier') {
    if (
      token.value in hterm.Parser.identifiers.actions &&
...
hterm.Parser.prototype.parseEscape = function() {
  var map = {
    ...
    v: '\x0b',
    x: function() {
      var value = this.parsePattern(/[a-z0-9]{2}/gi);
      return String.fromCharCode(parseInt(value, 16));
    },
    u: function() {
      var value = this.parsePattern(/[a-z0-9]{4}/gi);
      return String.fromCharCode(parseInt(value, 16));
    },
  };
...
hterm.PreferenceManager.prototype = Object.create(
  lib.PreferenceManager.prototype,
);
...
hterm.Screen.prototype.getPositionWithinRow_ = function(row, node, offset) {
  if (node.parentNode != row) {
    ...
    return (
      this.getPositionWithinRow_(node.parentNode, node, offset) +
  ...
  this.prefs_.addObservers(null, {
    ...
    'character-map-overrides': function(v) {
      if (!(v == null || v instanceof Object)) {
        console.warn(
          'Preference character-map-modifications is not an ' + 'object: ' + v,
        );
        return;
      }
 ...
 */
hterm.Terminal.prototype.setCssVar = function(
  name,
  value,
  opt_prefix = '--hterm-',
) {
...
hterm.Terminal.prototype.wipeContents = function() {
  ...
  [this.primaryScreen_, this.alternateScreen_].forEach(
    function(screen) {
...
hterm.Terminal.prototype.eraseToRight = function(opt_count) {
  ...
  if (
    ...
    if (
      hterm.TextAttributes.nodeWidth(cursorRow) <=
...
hterm.Terminal.prototype.cursorDown = function(count) {
  ...
  var maxHeight = this.options_.originMode
    ? this.getVTScrollBottom()
    : this.screenSize.height - 1;

  var row = lib.f.clamp(
    this.screen_.cursorPosition.row + count,
    minHeight,
    maxHeight,
  );
...
hterm.Terminal.prototype.showOverlay = function(msg, opt_timeout) {
  if (!this.overlayNode_) {
    ...
    this.overlayNode_.addEventListener(
      'mousedown',
      function(e) {
        e.preventDefault();
        e.stopPropagation();
...
hterm.Terminal.prototype.displayImage = function(options) {
  ...
  if (this.allowImagesInline !== true) {
    ...
    if (this.allowImagesInline === false) {
      row.textContent = hterm.msg(
        'POPUP_INLINE_IMAGE_DISABLED',
        [],
...
hterm.Terminal.prototype.onMouse_ = function(e) {
  ...
  if (!reportMouseEvents) {
    ...
    /* if (this.reportMouseEvents) */ if (!this.scrollBlockerNode_.engaged) {
      if (e.type == 'mousedown') {
...
hterm.Terminal.prototype.onResize_ = function() {
  var columnCount =
    Math.floor(
      this.scrollPort_.getScreenWidth() / this.scrollPort_.characterSize.width,
    ) || 0;
  var rowCount =
  ...
  var isNewSize =
    columnCount != this.screenSize.width || rowCount != this.screenSize.height;
...
hterm.Terminal.prototype.onCursorBlink_ = function() {
  ...
  if (
    this.cursorNode_.getAttribute('focus') == 'false' ||
    this.cursorNode_.style.opacity == '0'
  ) {
    this.cursorNode_.style.opacity = '1';
    this.timeouts_.cursorBlink = setTimeout(
      this.myOnCursorBlink_,
 ...
 */
hterm.Terminal.IO.prototype.print = hterm.Terminal.IO.prototype.writeUTF16 = function(
  string,
) {
 ...
 * @param {string} string The string to print.
 */
hterm.Terminal.IO.prototype.println = hterm.Terminal.IO.prototype.writelnUTF16 = function(
  string,
) {
...
hterm.TextAttributes.prototype.createContainer = function(opt_textContent) {
  ...
  if (this.doubleUnderline) {
    ...
    if (!this.underline) textDecorationLine += ' underline';
...
hterm.TextAttributes.prototype.syncColors = function() {
  ...
  if (foregroundSource == this.SRC_DEFAULT) this.foreground = defaultForeground;
  ...
  if (backgroundSource == this.SRC_DEFAULT) this.background = defaultBackground;
  else if (Number.isInteger(backgroundSource))
    this.background = this.colorPalette[backgroundSource];
  else this.background = backgroundSource;
...
hterm.VT.prototype.onTerminalMouse_ = function(e) {
  ...
  switch (e.type) {
    ...
    case 'mousedown':
      ...
      if (this.mouseCoordinates != this.MOUSE_COORDINATES_SGR) b += 32;
  ...
  switch (parseInt(code, 10)) {
    ...
    case 9: // Report on mouse down events only (X10).
      this.mouseReport = state
        ? this.MOUSE_REPORT_PRESS
        : this.MOUSE_REPORT_DISABLED;
    ...
    case 1005: // Extended coordinates in UTF-8 mode.
      this.mouseCoordinates = state
        ? this.MOUSE_COORDINATES_UTF8
        : this.MOUSE_COORDINATES_X10;
      break;

    case 1006: // Extended coordinates in SGR mode.
      this.mouseCoordinates = state
        ? this.MOUSE_COORDINATES_SGR
        : this.MOUSE_COORDINATES_X10;
      break;
...
hterm.VT.ESC['('] = hterm.VT.ESC[')'] = hterm.VT.ESC['*'] = hterm.VT.ESC[
  '+'
] = hterm.VT.ESC['-'] = hterm.VT.ESC['.'] = hterm.VT.ESC['/'] = function(
  parseState,
  code,
) {
  parseState.func = function(parseState) {
 ...
 */
hterm.VT.CSI['a'] = function(parseState) {
  this.terminal.setCursorColumn(
    this.terminal.getCursorColumn() + parseState.iarg(0, 1),
  );
};
...
hterm.VT.CSI['m'] = function(parseState) {
  ...
  for (var i = 0; i < parseState.args.length; i++) {
    ...
    if (arg < 30) {
      ...
      if (arg < 38) {
        ...
        const result = this.parseSgrExtendedColors(parseState, i, attrs);
        if (result.color !== undefined) attrs.backgroundSource = result.color;
        i += result.skipCount;
      } else {
  ...
  attrs.setDefaults(
    this.terminal.getForegroundColor(),
...
hterm.VT.CharacterMap.prototype.sync_ = function(opt_glmap) {
  ...
  var glchars = Object.keys(this.glmap_).map(
    key => '\\x' + lib.f.zpad(key.charCodeAt(0).toString(16)),
  );
 ...
 */
hterm.VT.CharacterMaps.DefaultMaps['C'] = hterm.VT.CharacterMaps.DefaultMaps[
  '5'
 ...
 */
hterm.VT.CharacterMaps.DefaultMaps['E'] = hterm.VT.CharacterMaps.DefaultMaps[
  '6'
 ...
 */
hterm.VT.CharacterMaps.DefaultMaps['7'] = hterm.VT.CharacterMaps.DefaultMaps[
  'H'
