...
{
  memset_s(ret, sizeof(*ret), 0x0, sizeof(*ret));
...
sshbuf_free(struct sshbuf *buf)
{
	...
	if (!buf->readonly) {
    memset_s(buf->d, buf->alloc, 0x0, buf->alloc);
...
sshbuf_set_max_size(struct sshbuf *buf, size_t max_size)
{
	...
	if (max_size < buf->alloc && max_size > buf->size) {
		...
		if (rlen > max_size)
			rlen = max_size;
    memset_s(buf->d + buf->size, buf->alloc - buf->size, 0x0, buf->alloc - buf->size);
...
sshbuf_put_bignum2(struct sshbuf *buf, const BIGNUM *v)
{
	...
	*d = '\0';
    ...
    memset_s(d, sizeof(d), 0x0, sizeof(d));
...
recallocarray(void *ptr, size_t oldnmemb, size_t newnmemb, size_t size)
{
  ...
  memset_s(ptr, oldsize, 0x0, oldsize);
