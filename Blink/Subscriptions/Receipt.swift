...
extension Publisher {
  func tap(_ handler: @escaping () -> () ) -> Publishers.HandleEvents<Self> {
...
class ReceiptMigrationProgress: ObservableObject {
  ...
  enum Status {
    case initial
    case locatingReciept
    case validatingReciept
    ...
    var recieptValidationFailed: Bool {
      switch self {
  ...
  func load() {
    recieptLocated = false
    recieptValidated = false
    migrationTokenGenerated = false
    state = .locatingReciept
    _receiptOperation =
      _skStore
...
enum ReceiptMigrationError: Error, LocalizedError {
  ...
  var errorDescription: String? {
    switch self {
    case .receiptExists(let msg): return "Reciept already exists"
    case .invalidAppReceipt(let msg): return "Invalid reciept"
